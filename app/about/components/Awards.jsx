...
export default function Awards() {
    ...
    if (inView) {
      const timer = setTimeout(() => {
        setVisibleIndexes((prevIndexes) => {
          const nextIndex = prevIndexes.length;
          if (nextIndex < awards.length + certificates.length) {
            return [...prevIndexes, nextIndex];
          } else {
            clearTimeout(timer);
            return prevIndexes;
          }
  ...
  }, [inView, visibleIndexes]);

  return (
    <div className="flex flex-col gap-14 items-center justify-center" ref={ref}>
      <div className="max-w-7xl w-11/12 flex flex-col gap-14 items-center justify-center h-auto ">
        <h1 className="text-[32px] md:text-[40px] font-normal font-montserrat text-center text-headingText mb-14">
          AWARDS & RECOGNITIONS
        </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-7xl w-full gap-x-8 gap-y-10 items-start justify-between">
            {awards.map((award, index) => (
              <AwardCard
                ...
                index={index}
                type="award"
                className={`h-auto transform transition-transform duration-500 ease-in-out ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 max-w-7xl w-full lg:grid-cols-4 gap-x-8 gap-y-10 items-start justify-between">
            {certificates.map((certificate, index) => (
              <AwardCard
                ...
                type="certificate"
                className={`h-auto transform transition-transform duration-500 ease-in-out ${
                  visibleIndexes.includes(index + awards.length) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                }`}
              />
            ))}
          </div>
