...
import { facSeriesData } from "../constants/ProductData"
      ...
      <div className="mt-3 font-montserrat  text-[15px] text-gray-700">
        {Object.entries(product.details).map(([key, value], index) => (
          <p key={index} className="grid grid-cols-2 justify-between gap-8">
            ...
            <span className="font-normal text-[14px] text-[#606467]">
              {value}
            </span>
          </p>
        ))}
...
const AwnProductCard = () => {
  const [selectedSeries, setSelectedSeries] = useState("Curtain Wall Facades");

  const currentSeries = facSeriesData.find(
